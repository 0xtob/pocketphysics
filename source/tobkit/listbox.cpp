...
void ListBox::penDown(u8 px, u8 py)
{
	...
	if(relx<width-SCROLLBAR_WIDTH) {
		...
		u8 newactiveelement = rel_item_clicked + scrollpos;
		...
		if(py<y+SCROLLBUTTON_HEIGHT) {
			...
			u8 rows_displayed = (height-1)/ROW_HEIGHT;
...
void ListBox::penMove(u8 px, u8 py)
{
	if(buttonstate==SCROLLTHINGY) {
		...
		u8 max_scrollthingypos = height-2*SCROLLBUTTON_HEIGHT-scrollthingyheight+2;
...
void ListBox::registerChangeCallback(void (*onChange_)(u8)) {
	onChange = onChange_;
}
...
void ListBox::ins(u8 idx, const char *name)
{
...
void ListBox::set(u8 idx, const char *name)
{
...
const char *ListBox::get(u8 idx) {
	return elements.at(idx).c_str();
}

u8 ListBox::getidx(void) {
