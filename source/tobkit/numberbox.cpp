...
NumberBox::NumberBox(u8 _x, u8 _y, u8 _width, u8 _height, uint16 **_vram, u8 _value, u8 _min, u8 _max)
	:Widget(_x, _y, _width, _height, _vram),
	value(_value), min(_min), max(_max), btnstate(0)
{
	onChange = 0;
}	
...
void NumberBox::draw(void)
{
	...
	// Number display
	drawFullBox(9,1,22,15,theme->col_lighter_bg);
	char *numberstr = (char*)malloc(4);
	sprintf(numberstr, "%3u", value);
