...
MessageBox::MessageBox(u16 **_vram, const char *message, u8 n_buttons, ...)
	...
	msg(message), n_buttons(n_buttons)
{
	...
	if(n_buttons > 0) {
		...
		for(u8 i=0; i<n_buttons; ++i) {
			...
			buttons[i] = new Button(xpos, y+24, buttonwidth, 14, _vram);
			gui.registerWidget(buttons[i], 0, MAIN_SCREEN);
...
void MessageBox::setTheme(Theme *theme_)
{
	gui.setTheme(theme_);
	theme = theme_;
}
